$(document).ready(function() {
    ...
    $('body').on("click", ".game .block", function() {
        if (exit_flag) { //erb krenq kam haxtenq kta true ev chi ashxati mer 
            return false;
        }
        let that = $(this)
        if ($(this).text() == '') {
    ...
    function rand_number() {
        find_val = false; //popoxakan urish dzev
        let val_div = div;
        let start_user = start;
        let qanak_div = qanak;

        //  SKZBIC KSTUGI LOGIKAN,aysinqn ete mnuma miat drvi ev haxti..ete chlini nor nerqevi kod@ kashxati patahakan tiv kntri--TOXOV@ miayn
    ...
    function find2(val_div, start_user, that) {
        if (find_val == false) {
            ...
            let popoxakan1 = 1;
            let o = 0;
            let div_number = parseInt(div); //kqcenq sra mej vor chisht ashxati--mer div@ cikli mej ashibkaya tali ete iranenq grum
            for (let i = data_div; i > 0; i -= div_number) {
                popoxakan = i;
            }
